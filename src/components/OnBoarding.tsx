...
export const OnBoarding = () => {
  ...
  const animationFrameTime = useRef<number>(0);
  ...
  useEffect(() => {
    ...
    const animate = (timestamp: number) => {
      if (!animationFrameTime.current) {
        animationFrameTime.current = timestamp;
      }

      const elapsedTime = timestamp - animationFrameTime.current;

      if (elapsedTime > 41.67) {
        requestId = requestAnimationFrame(() => {
          if (progressValue < 100) {
            ...
            setTimeout(() => {
              const divCurrent = componentRef.current!;
              if (divCurrent) {
  ...
  return (
    <div
      ref={componentRef}
      className={`fixed z-[9999] flex h-screen w-screen overflow-hidden flex-col items-center justify-end bg-[#d7d7d7] ease-in-out`}
    >
      <div ref={divRef} className="relative h-screen w-screen flex-col items-center justify-end flex overflow-hidden">
        <div
          ref={textRef}
          className={`absolute top-1/2 -translate-y-1/2 font-bold uppercase text-[#6b0e01] xs:text-[10px] md:text-3xl ${opacityText} ${scaleText} transition duration-[700ms] ease-in-out`}
        >
          đại việt sử ký
        </div>
        <div
        ...
        <div className="absolute bottom-16 flex flex-col items-center gap-6">
          <div
            style={{
              background: `conic-gradient(#6b0e01 ${
                progressValue * 3.6
              }deg, #ededed 0deg)`,
            }}
          ...
          <div className="animate reflect text-center flex">
